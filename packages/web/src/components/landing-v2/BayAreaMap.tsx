...
/**
 * Paint for the SVG fallback hero map (shown when MapLibre/WebGL is
 * unavailable). SVG presentation attributes do resolve `var()`, so tokens would
 * work here; these are literal so the fallback can mirror the HERO_PAINT ramp
 * in maplibreStyle.ts value-for-value and read as the same place in both
 * themes — the MapLibre spec next to it cannot use tokens.
 */
interface BayPaint {
  land: string;
  grid: string;
...
const BAY_PAINT: Record<ResolvedTheme, BayPaint> = {
  dark: {
    land: '#0B1220',
    grid: '#182540',
    waterInner: '#0A1628',
  ...
  light: {
    ...
    accent: '#B45309',
    green: '#047857',
    // Labels and the coastline highlight ride on very low opacities (0.06–0.7),
    // so the light ramp needs a much darker base to stay visible at all.
    label: '#475569',
    muted: '#64748B',
  },
};
...
export function BayAreaMap() {
  const paint = BAY_PAINT[useThemeStore((s) => s.resolved)];
    ...
    >
      <defs>
        ...
        <radialGradient id="bayGrad" cx="45%" cy="45%" r="55%">
          <stop offset="0%" stopColor={paint.waterInner} />
          <stop offset="100%" stopColor={paint.waterDeep} />
        </radialGradient>
        <pattern id="mapGrid" width="40" height="40" patternUnits="userSpaceOnUse">
          <path d="M 40 0 L 0 0 0 40" fill="none" stroke={paint.grid} strokeWidth="0.4" />
        </pattern>
      </defs>

      {/* Land base */}
      <rect width="1000" height="1100" fill={paint.land} />
      ...
      {/* Range rings */}
      <circle cx="410" cy="490" r="130" fill="none" stroke={paint.accent} strokeWidth="0.5" opacity="0.05" />
      <circle cx="410" cy="490" r="260" fill="none" stroke={paint.accent} strokeWidth="0.5" opacity="0.035" />
      <circle cx="410" cy="490" r="390" fill="none" stroke={paint.accent} strokeWidth="0.5" opacity="0.025" />
      ...
      <g fill="none" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path
          ...
          stroke={paint.highway} strokeWidth="2.5"
        />
        {/* I-80 */}
        <path
        ...
        <path
          ...
          stroke={paint.highway} strokeWidth="2"
        />
        {/* I-580 */}
        <path
        ...
        <path d="M305 548 L560 535" stroke={paint.highway} strokeWidth="1.5" strokeDasharray="6 4" />
        {/* Dumbarton Bridge */}
        <path d="M292 658 L572 648" stroke={paint.highway} strokeWidth="1.5" strokeDasharray="6 4" />
      </g>

      {/* Bridges — accent highlight */}
      <g stroke={paint.accent} strokeWidth="3" opacity="0.45" strokeLinecap="round" filter="url(#bridgeGlow)">
      ...
      <g fontFamily="'Space Grotesk',sans-serif">
        <circle cx="268" cy="258" r="3.5" fill={paint.label} opacity="0.6" />
        <text x="245" y="278" textAnchor="middle" fontSize="12" fontWeight="600" fill={paint.label} opacity="0.7">
          San Francisco
        </text>

        <circle cx="530" cy="282" r="3" fill={paint.label} opacity="0.5" />
        <text x="558" y="286" fontSize="11" fontWeight="500" fill={paint.label} opacity="0.6">Oakland</text>

        <circle cx="520" cy="195" r="2.5" fill={paint.label} opacity="0.4" />
        <text x="542" y="198" fontSize="10" fill={paint.label} opacity="0.5">Berkeley</text>

        <rect x="274" y="438" width="30" height="14" rx="2" fill="none" stroke={paint.muted} strokeWidth="0.8" opacity="0.4" />
        <text x="289" y="449" textAnchor="middle" fontSize="8" fontFamily="'JetBrains Mono',monospace" fill={paint.muted} opacity="0.5">SFO</text>

        <circle cx="448" cy="905" r="3" fill={paint.label} opacity="0.4" />
        <text x="448" y="928" textAnchor="middle" fontSize="11" fontWeight="500" fill={paint.label} opacity="0.5">San Jose</text>

        <circle cx="290" cy="680" r="2" fill={paint.label} opacity="0.3" />
        <text x="268" y="695" fontSize="10" fill={paint.label} opacity="0.4">Palo Alto</text>

        <circle cx="576" cy="680" r="2" fill={paint.label} opacity="0.3" />
        <text x="596" y="685" fontSize="10" fill={paint.label} opacity="0.4">Fremont</text>

        <circle cx="502" cy="118" r="2" fill={paint.label} opacity="0.3" />
        <text x="522" y="122" fontSize="10" fill={paint.label} opacity="0.4">Richmond</text>

        <circle cx="235" cy="345" r="1.8" fill={paint.label} opacity="0.25" />
        <text x="210" y="360" fontSize="9" fill={paint.label} opacity="0.35">Daly City</text>
      </g>

      {/* Animated driver dots */}
      <g filter="url(#driverGlow)">
        {/* Driver 1: 101 southbound SF->Peninsula — trail */}
        <circle r="3" fill={paint.accent} opacity="0.1">
        ...
        <circle r="3.5" fill={paint.accent} opacity="0.3">
          <animateMotion dur="24s" repeatCount="indefinite" begin="-0.35s"
            path="M260 255 C258 288,256 318,254 352 C252 395,254 440,258 485 C262 525,268 562,276 602 C286 648,298 692,315 735" />
        </circle>
        <circle r="4.5" fill={paint.accent} opacity="0.9">
        ...
        <circle r="4" fill={paint.green} opacity="0.8">
        ...
        <circle r="4" fill={paint.accent} opacity="0.8">
        ...
        <circle r="3.5" fill={paint.accent} opacity="0.7">
        ...
        <circle r="4" fill={paint.green} opacity="0.7">
        ...
        <circle r="4.5" fill={paint.accent} opacity="0.85">
        ...
        {/* Driver 8: South Bay */}
        <circle r="4" fill={paint.accent} opacity="0.75">
        ...
        <circle r="4" fill={paint.accent} opacity="0.85">
